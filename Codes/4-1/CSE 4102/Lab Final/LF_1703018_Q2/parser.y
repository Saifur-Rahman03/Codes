...
%token PRINT SCAN FUNC MAIN BEGIN END LET ELIF AS
%token ADDOP SUBOP MULOP DIVOP EQUOP LT GT INCAS
...
statements: statements statement | FUNC MAIN LPAREN RPAREN BEGIN para END;
...
dec:    LET ID AS INT
        ...
        ;
assin:  ID EQUOP ICONST
        | ID EQUOP ICONST ADDOP ID SUBOP ICONST 
        | ID INCAS ICONST
        | ID INCAS ICONST SUBOP ICONST
        | PRINT ID
        ;
...
cond:   ifcond
        | elifcond
        | elcond
ifcond: IF conds BEGIN assin END 
elifcond: ELIF conds BEGIN assin END
elcond: ELSE BEGIN assin END
