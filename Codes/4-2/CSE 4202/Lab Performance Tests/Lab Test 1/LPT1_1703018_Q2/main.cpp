...
int main()
{
    ...
    unsigned int fragmentShaderBlack = glCreateShader(GL_FRAGMENT_SHADER); // the third fragment shader that outputs the color Black

    unsigned int shaderProgramBlue = glCreateProgram();
    unsigned int shaderProgramGreen = glCreateProgram(); // the second shader program
    unsigned int shaderProgramBlack = glCreateProgram(); // the third shader program
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glDrawArrays(GL_TRIANGLES, 0, 3);	// this call should output a Black triangle
